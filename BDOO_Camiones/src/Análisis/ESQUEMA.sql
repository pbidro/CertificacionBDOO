...
CATEGORY VARCHAR(22);
	BEGIN
		SELECT MONTHS_BETWEEN(SYSDATE,FECHA_INGRESO)/12 INTO ANS FROM CHOFER;
...
TOTAL_VIAJES NUMBER;
	BEGIN
		...
		WHERE V.CHOFER.RUT=C.RUT AND C.ESTADO=1 AND V.ESTADO=1;
...
---PROBAR
-- SELECT COUNT(V.CODIGO) /*INTO TOTAL_VIAJES*/ FROM VIAJE V, CHOFER C WHERE V.CHOFER.RUT=C.RUT AND C.ESTADO=1 AND V.ESTADO=1;

select case 
 when months_between(sysdate,'22/10/2013')/12 < 2 then 'empleado nuevo'
 when months_between(sysdate,'22/10/2013')/12 < 5 then 'empleado medio'
  when months_between(sysdate,'22/10/2013')/12 >= 5 then 'empleado viejo'
  end from dual ;
